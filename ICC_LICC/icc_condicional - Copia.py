...
if int(posi_astro) > int(tam_matriz)**2:
    print(f"O astronauta ja saiu em missao ha {int(posi_astro) - (int(tam_matriz)**2)} chamadas.")


#Se não saiu, agora avaliar se está para sair
elif int(posi_astro) == int(tam_matriz)**2:
    if int(posi_astro) % 2 == 0:
        calculo_y = int(int(tam_matriz) / 2)
        calculo_x = (int(int(tam_matriz) / 2) -1)
        ...
        calculo_x_y = int((int(tam_matriz) / 2) - 0.5)
...
#Então agora é encontrar sua posição na fila e quantas chamadas faltam
    ...
    print(f"O astronauta esta na posicao: {0} {0}\nAinda faltam {(int(tam_matriz)**2)-int(posi_astro)} chamadas para a sua vez!")
